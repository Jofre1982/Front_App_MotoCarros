...
impl ApiClient {
    ...
    /// `GET /api/v1/sites` — historia #85 del backend. El pasajero lo usa
    /// para elegir el destino de un mandado (issue #76) del mismo catalogo
    /// que usa `RideEstimateScreen` para elegir el destino de un viaje (issue
    /// #74, en una rama aparte todavia no mergeada): un mandado no tiene
    /// tarifa fija, asi que solo hacen falta `id`/`name` de cada sitio.
...
mod tests {
    ...
    #[tokio::test]
    async fn list_sites_returns_the_catalog_for_the_destination_picker() {
        let server = MockServer::start().await;
